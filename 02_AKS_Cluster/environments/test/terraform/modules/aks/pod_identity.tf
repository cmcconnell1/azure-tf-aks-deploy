...
# Allow pod managed identity to access main existing env non-ephemeral key vault
# main env existing source key vault we copy secrets/certs from to ephemeral cluster keyvault
# Setup AKS Role Assignments for pod identity
#data "azurerm_resource_group" "aks_node_rg2" {
#  name = var.source_main_env_key_vault_resource_group
#}
#data "azurerm_key_vault" "source_main_env_key_vault_name" {
#  name                = var.source_main_env_key_vault_name
#  resource_group_name = var.source_main_env_key_vault_resource_group
#}
#
#resource "azurerm_key_vault_access_policy" "kv_podidentity_access_policy2" {
#  key_vault_id = data.azurerm_key_vault.source_main_env_key_vault_name.id
#  tenant_id    = data.azurerm_client_config.current.tenant_id
#  object_id    = azurerm_user_assigned_identity.podidentity.principal_id
#  key_permissions = [
#    "Get",
